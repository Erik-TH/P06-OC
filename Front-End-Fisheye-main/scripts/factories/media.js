 ...
 */
 class LikesCounter{
     ...
     * 
     * @param {*} media id 
     * @param {*} totalLikes for this photographer
     * @param {*} likes for this media
     */
    ...
    /**
     * likes counter increment
     * update total likes
     * @param {*} incr 
     * increment is relative integer
     */
    update(incr=1) {
        this._count += incr
...
/**
 * 
 * @param {*} data : json data for this media
 * @param {*} totalLikes : total likes for this photographer
 * @param {*} photografName : photographer's name - for url
 * @returns { id, photographerId, image, video, counter, date, title, getUserCardDOM, SetListeners, LightBoxRender }
 * 3 returns
 * getUserCardDOM() : render thumbnail
 * setLIsteners() : listeners for this media
 * LightBoxRender() : lightbox render 
 */
function mediaFactory(data,totalLikes,photografName) {    
    ...
    let sectionHTML = ""
    const picture = BaseURL.base + `public/assets/photographers/${name.split(' ')[0].replace('-',' ')}/${image?image:video}`;
     ...
     * 
     * @returns {article}
     * html element and childs
     */
    function getUserCardDOM() {
        // if thumbnail already done return html already built
        if(sectionHTML !== ""){

            return sectionHTML
        }
        
        // photographer's thumbnail
        const article = document.createElement( 'article' );
        article.setAttribute("id", "media-"+id)
        article.setAttribute("aria-label", title)

        // element <a> which contain the media and will start caroussel
        ...
        // determine if thumbnail video or photo
        ...
        // if video
        if(video){
            // no player controls for videos on mainpage
    ...
    /** 
     * 
     * @returns {string} return frag HTML lightbox for media
     */
    ...
    /**
     * 
     * @param {*} el : element - switch between class like not liked
     */
